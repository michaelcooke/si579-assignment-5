...
function addToSavedWords(word) {
  if (savedWordsArray.indexOf(word) === -1) {
...
function getRhymes() {
  ...
  datamuseRequest(getDatamuseRhymeUrl(wordInput.value), (rhymes) => {
    wordOutput.innerHTML = ''
    outputDescription.innerHTML = `Words that rhyme with ${wordInput.value}`
    const rhymeGroups = groupBy(rhymes, 'numSyllables')
    content = ''
    if (Object.keys(rhymeGroups).length > 0) {
      Object.keys(rhymeGroups).forEach((numSyllables) => {
        content += `<h3>Syllables: ${numSyllables}</h3>`
        content += `<ul>`
        rhymeGroups[numSyllables].forEach((word) => {
          content += `<li>${word.word}<button class="btn btn-outline-success" onclick="addToSavedWords('${word.word}')">(save)</button></li>`
        })
        content += `</ul>`
      })
    } else {
...
function getSynonyms() {
  ...
  wordOutput.innerHTML += '<span>...loading</span>'
  datamuseRequest(getDatamuseSimilarToUrl(wordInput.value), (synonyms) => {
    wordOutput.innerHTML = ''
    outputDescription.innerHTML = `Words with a meaning similar to ${wordInput.value}`
    content = ''
    if (synonyms.length > 0) {
      ...
      synonyms.forEach((synonym) => {
        content += `<li>${synonym.word}<button class="btn btn-outline-success" onclick="addToSavedWords('${synonym.word}')">(save)</button></li>`
      })
      content += `</ul>`
    } else {
      content += `<span>(no results)</span>`
